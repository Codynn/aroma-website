      ...
      {video.videoId && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="bg-black/60 p-3 rounded-full backdrop-blur-sm group-hover:bg-black/80 transition-all">
             <svg className="w-10 h-10 sm:w-12 sm:h-12 text-white fill-current" viewBox="0 0 24 24">
...
export default function TeaStoriesSection() {
  ...
  return (
    <section aria-labelledby="tea-stories-heading" className="relative w-full py-12 sm:py-16 lg:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <h2 id="tea-stories-heading" className="font-sora font-bold text-[#77923B] text-center text-[1.6rem] sm:text-[2rem] lg:text-[2.5rem] leading-tight mb-2">
        ...
        {/* ── DESKTOP (lg+) ── */}
        <div className="hidden lg:grid grid-cols-3 gap-4 items-stretch" style={{ height: '580px' }}>
          <VideoCard video={v1} onOpen={setActiveVideo} className="h-full" />
          <div className="flex flex-col gap-4 h-full">
            <FollowUs label={TEA_STORIES_CONTENT.followLabel} socials={TEA_STORIES_SOCIALS} />
            <VideoCard video={v2} onOpen={setActiveVideo} className="flex-1 min-h-0" />
          </div>
          <VideoCard video={v3} onOpen={setActiveVideo} className="h-full" />
        </div>

        {/* ── MOBILE (<lg) ── */}
        <div className="lg:hidden grid grid-cols-3 gap-2 sm:gap-3 items-stretch" style={{ height: '340px' }}>
          <VideoCard video={v1} onOpen={setActiveVideo} className="h-full" />
            ...
            <div className="flex flex-col items-center justify-center gap-2 rounded-2xl bg-white px-2 py-3 shrink-0">
              ...
              <div className="flex items-center gap-3">
                {TEA_STORIES_SOCIALS.map((s) => (
                  <Link key={s.id} href={s.href} target="_blank" rel="noopener noreferrer" className="hover:opacity-75 transition-opacity duration-200">
                    <Image src={`/icons/${s.icon}.svg`} alt={s.label} width={22} height={22} className="w-5 h-5 sm:w-6 sm:h-6 object-contain" />
                  </Link>
                ))}
              </div>
            </div>
