...
import { Star, RefreshCcw, Loader2, Kanban } from "lucide-react";
  ...
  return (
    <div className="max-w-7xl flex flex-col items-center justify-center mx-auto px-4 min-h-screen">
      ...
      <div className="w-full flex justify-between items-center mb-12">
        <button className="flex items-center gap-3 px-[14px] py-2 md:px-6 cursor-pointer md:py-2.5 border border-[#7D8F7B] rounded-[16px] hover:bg-gray-100 transition-all">
          <Kanban
            width={19}
            strokeWidth={2.5}
            height={19}
            className="-rotate-90"
          />
          <span className="font-sora text-[12px] md:text-[18px]">
            Open Filter
          </span>
        </button>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="mt-4 font-sora text-gray-500 text-[18px]">
            Fetching Himalayan teas...
          </p>
        </div>
      ) : isError ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <h2 className="font-sora font-bold text-2xl text-red-600 mb-4">
            Failed to load products
          </h2>
          <button
            onClick={() => refetch()}
            className="px-8 py-3 bg-[#77923B] text-white rounded-full font-sora font-bold"
          >
            Try Again
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-4 md:gap-y-8 w-full mb-6 lg:mb-16">
            {products.map((product) => (
              ...
              >
                ...
                <div
                  className="relative overflow-hidden rounded-[16px] bg-gray-50 
                                w-[171px] h-[225px] 
                ...
                <div className="flex justify-between items-start mb-1 px-1 w-full">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-sora font-medium text-[12px] md:text-[14px] text-black">
                      5.0
                    </span>
                  </div>
                </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
          ...
          <h2 className="font-sora font-bold text-[24px] md:text-[32px] text-gray-900 mb-2">
            No Data Found
          </h2>
          <p className="font-sora text-gray-500 mb-8 max-w-md">
            We couldn't find any teas matching your current filters.
          </p>
          <button
            onClick={() => refetch()}
            className="flex items-center gap-2 px-8 py-3 bg-[#77923B] text-white rounded-full font-sora font-bold hover:bg-[#667d32] transition-colors"
          >
